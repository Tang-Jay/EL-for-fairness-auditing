...
#===============================================
P14_value<-function(group_specific_data,target=theta_P,epsilon0=0.05){
  # 超参数赋值
  m <- 1
  target <- theta_P
  ...
  if(mean(L1) < epsilon0){T14 <- T04} else {T14 <- 0}
  
  # 调包计算
  # library(emplik)
  # if (mean(L1) <= epsilon0) {
  #   # 如果估计值小于等于epsilon0，T14 = 0
  #   T14 <- 0
  # } else {
  #   # 检验 H0: epsilon = epsilon0
